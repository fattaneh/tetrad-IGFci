 ...
 */
public final class ISMlBayesIm implements BayesIm {
	 ...
	 */
	public int[][] parents;
	public Map<Node, Map<List<Integer>, List<String>>> nodeCSI = new HashMap<>();
	 ...
	 */
	public ISMlBayesIm(BayesPm bayesPm) throws IllegalArgumentException {
	 ...
	 *                                  the bayes parametric model provided.
	 */
	public ISMlBayesIm(BayesPm bayesPm, int initializationMethod)
	 ...
	 *                                  the bayes parametric model provided.
	 */
	public ISMlBayesIm(BayesPm bayesPm, BayesIm oldBayesIm,
		...
		// Initialize.
		initializeIS(oldBayesIm, initializationMethod);
	}

	/**
	 * Copy constructor.
	 */
	public ISMlBayesIm(BayesIm bayesIm) throws IllegalArgumentException {
		if (bayesIm == null) {
		...
		initializeIS(bayesIm, MlBayesIm.MANUAL);
	}
	...
	private static double[] getRandomWeights3(int size) {
		...
		for (int i = 0; i < size; i++) {
			...
			double exaggerationFactor = 3;

			if (v > 0.5) v *= exaggerationFactor;
			if (v < 0.5) v /= exaggerationFactor;
...
//			System.out.println("parents: "+parentList);

			for (Node p : parentList){
				List<String> categories = new ArrayList<String>();
				for (int c = 0; c < getBayesPm().getNumCategories(p); c++){
					categories.add(getBayesPm().getCategory(p, c));
				}
				parentMap.put(getNodeIndex(p), categories);
			}
...
//				System.out.println(parentMap);
				...
				List<String> isValues = new ArrayList<String>();
				for (int s = 0; s < isParents.size(); s ++){
					if (parentMap.get(isParents.get(s)).size() > 0){
...
//			System.out.println("key: " + key);
			for (Entry<List<Integer>, List<String>> p: this.nodeCSI.get(node).entrySet()){
				int[] parentArrayIS = new int[p.getKey().size()];
				for (int i = 0; i < p.getKey().size(); i++) {
					parentArrayIS[i] = p.getKey().get(i);
				}		
				// Sort parent array.
				Arrays.sort(parentArrayIS);
//				for (int i = 0; i < parentArrayIS.length; i++) {
//				valuesIS[parentArrayIS[i]] = Integer.parseInt(p.getValue().get(p.getKey().indexOf(parentArrayIS[i])));
//			}
				int[] valuesIS = new int[parents[nodeIndex].length];

				for (int i = 0; i < parents[nodeIndex].length; i++) {
					if(p.getKey().contains(parents[nodeIndex][i])){
						valuesIS[i] = Integer.parseInt(p.getValue().get(p.getKey().indexOf(parents[nodeIndex][i])));
					}
				}
//				System.out.println("IS array: " + Arrays.toString(parentArrayIS));
//				System.out.println("IS values: " +Arrays.toString(valuesIS));

				// compute all combinations of the non IS parents with the IS ones
				int[] parentArrayRemoved = new int[graph.getParents(node).size()-p.getKey().size()];
				List<Node> removedParents = new ArrayList<Node>();

				int j = 0;
				for (Node n: graph.getParents(node)) {
					int idx = getNodeIndex(n);
					if (!p.getKey().contains(idx)){
						removedParents.add(n);
						parentArrayRemoved[j] = idx;
						j++;
					}
				}
				Arrays.sort(parentArrayRemoved);
//				System.out.println("removed: " + Arrays.toString(parentArrayRemoved));

				int[] dims = new int[parentArrayRemoved.length];
				int dimProd = 1;
				for (int i = 0; i < dims.length; i++) {
					Node parNode = nodes[parentArrayRemoved[i]];
...
//				System.out.println("dims: " + Arrays.toString(dims));
				...
				for (int row = 0; row < dimProd; row ++){
					...
					int idx = 0;
					for (int k = 0 ; k < parents[nodeIndex].length; k++){
						Node n = nodes[parents[nodeIndex][k]];
						if(removedParents.contains(n)){
							valuesIS[k] = values[idx];
							idx ++;
						}
					}	
//					System.out.println(Arrays.toString(valuesIS));
					
					if (firstTime){
						rowIndex = getRowIndex(nodeIndex, valuesIS);
						randomizeRow(nodeIndex, rowIndex);
						pRow = probs[nodeIndex][rowIndex];
						firstTime = false;
					}
					else{
						
						probs[nodeIndex][getRowIndex(nodeIndex, valuesIS)]= pRow;
					}
				}
			}
		}
