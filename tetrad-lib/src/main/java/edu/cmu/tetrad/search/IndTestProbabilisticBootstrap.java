 ...
 *
 * @author Fattaneh Agu 21, 2019
 */
public class IndTestProbabilisticBootstrap implements IndependenceTest {
	 ...
	 */
	public IndTestProbabilisticBootstrap(DataSet[] dataSets, int numBootstrapSamples) {

		this.dataSets = dataSets;
		this.numBootstrapSamples = numBootstrapSamples;
		this.bci = new BCInference[numBootstrapSamples];
		for (int bs = 0 ; bs < dataSets.length ; bs++){
			if (!dataSets[bs].isDiscrete()) {
				throw new IllegalArgumentException("Not a discrete data set.");
			}

			int[] nodeDimensions = new int[dataSets[bs].getNumColumns() + 2];

			for (int j = 0; j < dataSets[bs].getNumColumns(); j++) {
				DiscreteVariable variable = (DiscreteVariable) (dataSets[bs].getVariable(j));
				int numCategories = variable.getNumCategories();
				//            System.out.println("Variable " + variable + " # cat = " + numCategories);
				nodeDimensions[j + 1] = numCategories;
			}

			int[][] cases = new int[dataSets[bs].getNumRows() + 1][dataSets[bs].getNumColumns() + 2];

			for (int i = 0; i < dataSets[bs].getNumRows(); i++) {
				for (int j = 0; j < dataSets[bs].getNumColumns(); j++) {
					cases[i + 1][j + 1] = dataSets[bs].getInt(i, j) + 1;
				}
			}

			bci[bs] = new BCInference(cases, nodeDimensions);

			nodes = dataSets[bs].getVariables();
		}
	...
	public double probConstraint(BCInference.OP op, Node x, Node y, Node[] z) {
		...
		double[] probs = new double[this.numBootstrapSamples];
		double prob = 0.0;
		for (int bs = 0; bs < this.numBootstrapSamples; bs++){
			probs[bs] = this.bci[bs].probConstraint(op, _x, _y, _z);
			prob += probs[bs];
		}
//		System.out.println("probs: " + Arrays.toString(probs));
		prob = prob / this.numBootstrapSamples;
//		System.out.println("avg p: " + prob);
