...
public class RBExperiments_sim_contiuous {
	...
	public static void main(String[] args) throws IOException {
		...
		Long seed = 1454147771L;
		double alpha = 0.05, cutoff = 0.5, lower = 0.3, upper = 0.7;		
		int numVars = 20, numCases = 200, numModels = 100, numBootstrapSamples = 500, round = 0, numSim = 1;
		double edgesPerNode = 4.0, samplePrior = 1.0;
		boolean threshold1 = false, threshold2 = true;
	...
	public void experiment(int numModels ,double alpha, boolean threshold1, boolean threshold2, double cutoff, 
		...
		try {
			File dir = new File(data_path+ "/simulation-" + RBExperiments_sim_contiuous.algorithm + 
					"-depth"+ RBExperiments_sim_contiuous.depth + "/");
			if (!threshold1){
				cutoff = Double.NaN;
			}
			dir.mkdirs();
			String outputFileName = "V"+numVars +"-E"+ edgesPerNode +"-L"+ latent + "-N" + numCases + "-M" 
					...
					RBExperiments_sim_contiuous.algorithm  + "-round"+ round + ".csv";
		...
		for (int s = 0; s < numSim; s++){
			...
			try {
				File dir = new File(data_path+ "/simulation-" + RBExperiments_sim_contiuous.algorithm + 
						"-depth"+ RBExperiments_sim_contiuous.depth +"/logfiles-V" + numVars +"-E"+ 
						edgesPerNode +"-L"+ latent + "-N" + numCases + "-M" + numModels + "-BS" + 
						numBootstrapSamples);
				dir.mkdirs();
				if (!threshold1){
					cutoff = Double.NaN;
				...
				String logFileName = "V"+numVars +"-E"+ edgesPerNode +"-L"+ latent + "-N" + numCases  +
						"-M" + numModels + "-Th1" + threshold1  + "-C" + cutoff + "-BS" + numBootstrapSamples 
						+"-S" +s + "-" + RBExperiments_sim_contiuous.algorithm  + "-round"+ round + ".log";
	...
	private DataSet createDepDataFiltering(Map<IndependenceFact, Double> H, DataSet data, int numBootstrapSamples,
		...
		for (int b = 0; b < numBootstrapSamples; b++) {
			...
			IndTestProbabilisticSemBic bsTest = new IndTestProbabilisticSemBic(bsData);
