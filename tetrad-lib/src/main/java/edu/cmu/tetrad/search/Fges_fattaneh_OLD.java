 ...
 */
public final class Fges_fattaneh_OLD implements GraphSearch, GraphScorer {
	...
	private boolean verboose2 = false;
	 ...
	 */
	public Fges_fattaneh_OLD(Score score) {
		if (score == null) {
	...
	public Graph search() {
		...
		if (faithfulnessAssumed) {
			if (verboose2)
				System.out.println("initializeForwardEdgesFromEmptyGraph started!");
			
			initializeForwardEdgesFromEmptyGraph(getVariables());
			
			if (verboose2)
			System.out.println("initializeForwardEdgesFromEmptyGraph done!");

			// Do forward search.
			if (verboose2)
				System.out.println("----- heuristicSpeedup in faithfulness! -----");
			this.mode = Mode.heuristicSpeedup;
			fes();
			bes();

			if (verboose2)
				System.out.println("----- coverNoncolliders in faithfulness! -----");
			...
			// Do forward search.
			if (verboose2)
				System.out.println("----- heuristicSpeedup in NOT faithfulness! -----");
			...
			if (verboose2)
				System.out.println("----- allowUnfaithfulness in NOT faithfulness! -----");
...
//								System.out.println("sortedArrows after calculateArrowsForward IN REVERSE : " +  sortedArrows);
					...
					if (symmetricFirstStep){
						if (bump2 > 0) {
							if (initialGraph == null ){
								addArrow(y, x, emptySet, emptySet, bump2);
	...
	private void bes() {
		...
		while (!sortedArrows.isEmpty()) {
			...
			if (deleted && verboose2){
				System.out.println("delete " + arrow);
				System.out.println(graph.getEdges());
				System.out.println("bump:" + bump);
			}
			if (!deleted) {
...
//		System.out.println("calculateArrowsForward");
		if (mode == Mode.heuristicSpeedup && !effectEdgesGraph.isAdjacentTo(a, b)) {
			System.out.println("mode == Mode.heuristicSpeedup && !effectEdgesGraph.isAdjacentTo(a, b)");
			return;
		}
		if (adjacencies != null && !adjacencies.isAdjacentTo(a, b)) {
			System.out.println("adjacencies != null && !adjacencies.isAdjacentTo(a, b)");
